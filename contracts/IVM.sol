...
// prevrandao
    ...
    function setUserAsValidator(address _userAddress) external onlyOwner {
        isValidator[_userAddress] = true;
    }

    function generateUUID(address _address) internal view returns (string memory) {
        bytes32 uuid = keccak256(abi.encodePacked(block.timestamp, block.prevrandao, _address));
        return bytes12ToHexString(bytes12(uuid));
    }

    function bytes12ToHexString(bytes12 _bytes12) internal pure returns (string memory) {
    ...
    function verifyUser(address _userAddress) external {
        require(isValidator[msg.sender] || msg.sender == owner, "Only the owner or a validator can verify users");
    ...
    event AccountCreated(address indexed userAddress, string name, uint256 age, string addr, string image);
    event AccessRequested(address indexed requester, address indexed requestedUser);
    event AccessGranted(address indexed requested, address indexed requestedUser);

    function createUser(string memory _name, uint256 _dateOfBirth, string memory _addr, string memory _image) external {
        require(bytes(_name).length > 0, "Name must not be empty");
        require(bytes(_addr).length > 0, "Address must not be empty");
        require(!userExists[msg.sender], "User already exists");

        require(_dateOfBirth <= block.timestamp - 220898880,"User must be atleast 7 years old");

        address accountAddress = msg.sender;
        bool _isVerified = false;
        string memory _validationToken = '';
        users[accountAddress] = userInfo(_name, _dateOfBirth, _addr, _image, _isVerified, _validationToken);
    ...
    function sendAccessRequest(address _requestedUser) external {
        ...
        requests[_requestedUser].push(Request(requester, _requestedUser, false, 0, 0));
        emit AccessRequested(requester, _requestedUser);
    }

    function grantAccess(address _requester, uint256 _customAccessTime) external {
    ...
    function getUserInfo(address _userAddress) external view returns (userInfo memory) {
        ...
        require(allowedToAccess[msg.sender][_userAddress] && isWithinTimeLimit(requests[_userAddress][0].timestamp, accessTime), "Access not granted or time limit exceeded");

        return users[_userAddress];
    }

    function isWithinTimeLimit(uint256 _timestamp, uint256 _timeLimit) internal view returns (bool) {
